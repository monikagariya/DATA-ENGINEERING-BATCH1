...
select suppliername from supplier WHERE EXISTS (SELECT ProductName FROM Product WHERE Product.SupplierID = Supplier.supplierID );

select suppliername,address from supplier where supplierid = any(select productid from product where categoryid = 2);
